#!/usr/bin/env python3
"""Generate and export compact TPC-DS Q85 chain3 datasets by scale factor."""
...
from export_query85_chain3 import export_case, scale_label
...
def ensure_scale_data(tools_dir: Path, data_root: Path, scale: str, source_sf1: Path | None) -> Path:
    ...
    for table in REQUIRED_TABLES:
        if not (data_dir / f"{table}.dat").exists():
            generate_table(tools_dir, data_dir, scale, TABLE_GENERATORS.get(table, table))
            if not (data_dir / f"{table}.dat").exists():
                raise FileNotFoundError(f"missing {data_dir / f'{table}.dat'} after generating table {table}")
    return data_dir


def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare TPC-DS Q85 chain3 projected datasets.")
