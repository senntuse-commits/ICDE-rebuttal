...
def ensure_dsdgen(tools_dir: Path) -> Path:
    ...
    if not dsdgen.exists():
        ...
        # A working tree copied from Windows can contain CRLF distribution
        # files. The old TPC-DS distcomp parser can crash on those files, so
        # normalize its text inputs before the native Linux build.
        for pattern in ("*.dst", "*.txt"):
