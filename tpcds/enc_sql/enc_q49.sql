   ...
   from (
     ...
     from (
       select 
         ...
         (sum(wr.wr_return_quantity) / sum(ws.ws_quantity)) as return_ratio,
         (sum(wr.wr_return_amt) / sum(ws.ws_net_paid)) as currency_ratio
       from 
         web_sales ws 
   ...
   from (
     ...
     from (
       select 
         ...
         (sum(cr.cr_return_quantity) / sum(cs.cs_quantity)) as return_ratio,
         (sum(cr.cr_return_amount) / sum(cs.cs_net_paid)) as currency_ratio
       from 
         catalog_sales cs 
   ...
   from (
     ...
     from (
       select 
         ...
         (sum(sr.sr_return_quantity) / sum(sts.ss_quantity)) as return_ratio,
         (sum(sr.sr_return_amt) / sum(sts.ss_net_paid)) as currency_ratio
       from 
         store_sales sts 
